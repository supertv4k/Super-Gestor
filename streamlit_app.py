...
with tab1:
    ...
    if not df.empty:
        ...
        for _,r in df_f.iterrows():
            cls = "row-vencido" if r['dias_res'] < 0 else "row-hoje" if r['dias_res']==0 else "row-amanha" if r['dias_res']==1 else "row-em-dia"
    ...
    with st.form("form"):
        ...
        if st.form_submit_button("SALVAR"):
            ...
            try:
                ...
                # 🔥 CORREÇÃO PRINCIPAL (EVITA IntegrityError)
                inicio_f = inicio.strftime("%Y-%m-%d") if inicio else datetime.now().strftime("%Y-%m-%d")
                venc_f = venc.strftime("%Y-%m-%d") if venc else datetime.now().strftime("%Y-%m-%d")
...
with tab3:
    ...
    if not df.empty:
        ...
        if st.button("WhatsApp"):
            for i in sel:
                msg = f"Olá {i['nome']}, vence {format_data_br(i['vencimento'])}"
                st.link_button("Enviar", f"https://wa.me/55{i['whatsapp']}?text={urllib.parse.quote(msg)}")

# ================= BACKUP BLINDADO (SEM ERRO)
with tab4:
    st.subheader("Backup")

    if st.button("BAIXAR LISTA"):
