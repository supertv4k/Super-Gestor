...
st.markdown("""
    ...
    /* Logo do Servidor no Card */
    .img-servidor { width: 55px; height: 55px; border-radius: 10px; object-fit: cover; border: 1px solid #444; }

    /* BOTÃO RETANGULAR E ALONGADO (SOLUÇÃO) */
    div.stButton > button {
        ...
        width: 100% !important; /* Alonga o botão na horizontal */
...
with tab4:
    ...
    with col_aj1:
        ...
        if st.button("🔄 FORÇAR SINCRONIZAÇÃO AGORA"):
            ...
            st.rerun()
        st.markdown("---")
        st.download_button(label="📥 BAIXAR PLANILHA (CSV)", data=df.to_csv(index=False).encode('utf-8-sig'), file_name=f"backup_supertv_{datetime.now().strftime('%d_%m_%Y')}.csv", mime="text/csv")
    with col_aj2:
