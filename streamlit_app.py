...
with tab1:
    ...
    # Lógica para "lembrar" qual cliente está sendo editado
    ...
    if not df.empty:
        ...
        for _, r in df_f.sort_values(by='nome').iterrows():
            ...
            with col_btn:
                ...
                if st.button(label, key=f"btn_{r['id']}", use_container_width=True):
                    st.session_state.cliente_editando = r['id']

            # Se este for o cliente selecionado, abre o formulário de edição
            if st.session_state.cliente_editando == r['id']:
                with st.expander(f"📝 EDITAR: {str(r['nome']).upper()}", expanded=True):
                    with st.form(key=f"form_edicao_{r['id']}"):
                        ...
                        lista_serv = get_servidores()
                        idx_serv = lista_serv.index(r['servidor']) if r['servidor'] in lista_serv else 0
                        ed_serv = ce1.selectbox("SERVIDOR", lista_serv, index=idx_serv)
                        ...
                        # BOTÕES DE AÇÃO
                        b_salvar = st.form_submit_button("💾 GRAVAR ALTERAÇÕES", use_container_width=True)
                        ...
                        if b_salvar:
                            ...
                            st.session_state.cliente_editando = None # Fecha o editor
                            st.rerun()

                    # Opções extras fora do form
                    col_excluir, col_cancelar = st.columns(2)
                    if col_excluir.button("🗑️ EXCLUIR CLIENTE", key=f"del_{r['id']}", type="primary"):
                        conn = sqlite3.connect('supertv_gestao.db')
                    ...
                    if col_cancelar.button("✖️ FECHAR SEM SALVAR", key=f"close_{r['id']}"):
                        st.session_state.cliente_editando = None
                        st.rerun()
...
with tab3:
    ...
    if not df.empty:
        ...
        for _, c in df_c.sort_values(by='dias_res').iterrows():
            ...
            if col_ch.checkbox("", key=f"sel_cob_{c['id']}"): 
                clientes_sel.append(c)
        ...
        if st.button(f"📲 GERAR LINKS DE COBRANÇA ({len(clientes_sel)})", use_container_width=True):
    ...
    with aj1:
        ...
        if st.button("➕ ADICIONAR SERVIDOR"):
            if ns:
                ...
                st.rerun()
    with aj2:
        if st.button("📦 EXPORTAR EXCEL (BACKUP)"):
            out = io.BytesIO()
            df.to_excel(out, index=False)
            st.download_button("⬇️ Baixar Backup", out.getvalue(), "backup_supertv.xlsx")
