    ...
    with tab3:
        if not st.session_state.modo_fila_ativo:
            ...
            if not df_c.empty:
                ...
                for _, r in df_c.iterrows():
                    ...
                    with st.container():
                        ...
                        is_sel = c1.checkbox("", value=sel_all, key=f"chk_{r['id']}")
                        if is_sel: clientes_marcados.append(r.to_dict())
                ...
                if st.button("🚀 INICIAR FILA DE DISPARO", type="primary"):
                    if clientes_marcados:
                        ...
                        st.session_state.indice_fila = 0
                        st.session_state.modo_fila_ativo = True; st.rerun()
                    else: st.error("Selecione ao menos um cliente!")
        else:
            # --- MODO FILA ATIVO ---
            ...
            if idx < len(fila):
                ...
                # Suas mensagens e PIX preservados exatamente
                cnpj_pix = "\n\n💠PIX CNPJ\n62.326.879/0001-13\n\n⚠️ NÃO ESQUEÇA DE ENVIAR O COMPROVANTE NO WHATSAPP!!!"
                msg_map = {
                    "vencidos": "🚨SUA ASSINATURA DE TV VENCEU !" + cnpj_pix,
                    "hoje": "⚠️SUA ASSINATURA DE TV VENCE HOJE ⏰!" + cnpj_pix,
                ...
                if col1.link_button("📲 ENVIAR AGORA", url_whats, type="primary"): st.session_state.em_disparo = True
                if col2.button("⏭️ PULAR"): st.session_state.indice_fila += 1; st.rerun()
                if col3.button("✖️ PARAR ENVIO"): st.session_state.modo_fila_ativo = False; st.rerun()

                if st.session_state.em_disparo:
                    st.warning("⏱️ Aguarde 10 segundos de segurança...")
                    barra = st.progress(0)
                    for t in range(10):
                        time.sleep(1); barra.progress((t + 1) * 10)
                    st.session_state.indice_fila += 1
                    st.session_state.em_disparo = False; st.rerun()
            else:
