...
# --- 2. ESTILIZAÇÃO CSS (VISUAL EXTERNO DA LOGO) ---
st.markdown("""
    <style>
    ...
    /* Ajuste para o expander de edição não parecer um botão separado */
...
if not df.empty:
    ...
    c1.markdown(f'<div class="metric-card">TOTAL: <span class="metric-value">{len(df)}</span></div>', unsafe_allow_html=True)
    c2.markdown(f'<div class="metric-card">VENCIDOS: <span class="metric-value">{len(df[df["dias_res"] < 0])}</span></div>', unsafe_allow_html=True)
    c3.markdown(f'<div class="metric-card">LUCRO: <span class="metric-value">R$ {(df["mensalidade"].sum() - df["custo"].sum()):,.2f}</span></div>', unsafe_allow_html=True)
    ...
    if not df.empty:
        for _, r in df.sort_values(by='nome').iterrows():
            if busca.lower() in r['nome'].lower() or busca.lower() in str(r['usuario']).lower():
                ...
                # --- O DIFERENCIAL: LOGO E INFO FORA DO BOTÃO ---
                st.markdown(f"""
                ...
                # O expander agora serve apenas como o botão de "EDITAR/VER MAIS"
...
with tab2:
    ...
    with st.form("add"):
        ...
        n_m = st.number_input("VALOR", value=35.0)
