...
# --- 2. ESTILIZAÇÃO CSS (DESIGN DOS CARDS - PRESERVADO) ---
st.markdown("""
    <style>
...
if not df.empty:
    ...
    with tab3:
        # MODO DE DISPARO EM MASSA ATIVO
        if st.session_state.executando_disparo:
            ...
            if idx < len(fila):
                ...
                # Mensagens baseadas no filtro usado na seleção
                ...
                if col1.link_button("📲 ENVIAR AGORA", url_whats, type="primary"):
                    st.session_state.aguardando_proximo = True
                
                if col2.button("⏭️ PULAR"):
                    st.session_state.indice_disparo += 1
                    st.rerun()
                    
                if col3.button("✖️ PARAR DISPARO"):
                    st.session_state.executando_disparo = False
                    st.rerun()

                if st.session_state.get('aguardando_proximo'):
                    st.info("⏱️ Aguardando 10 segundos para o próximo disparo...")
                    barra = st.progress(0)
                    for i in range(10):
                        time.sleep(1)
                        barra.progress((i + 1) * 10)
                    st.session_state.indice_disparo += 1
                    st.session_state.aguardando_proximo = False
                    st.rerun()
            else:
                st.success("✅ Fila de disparos finalizada!")
                if st.button("VOLTAR PARA LISTA"):
                    st.session_state.executando_disparo = False
                    st.rerun()

        else:
            # INTERFACE DE SELEÇÃO NORMAL
            ...
            if not df_c.empty:
                ...
                if col_btn2.button("🚀 ENVIAR EM MASSA", type="primary"):
                    if clientes_marcados:
                        ...
                        st.session_state.indice_disparo = 0
                        st.session_state.executando_disparo = True
                        st.rerun()
