...
# Inicialização da Fila Selecionável
...
if not df.empty:
    ...
    with tab3:
        if not st.session_state.modo_fila_ativo:
            st.subheader("🚨 SELECIONAR CLIENTES PARA COBRANÇA")
            c_cols = st.columns(6)
            filtros = ["vencidos", "hoje", "1dia", "2dias", "3dias", "todos"]
            labels = ["❌ Vencidos", "📅 Hoje", "🌅 Amanhã", "⏳ 2 Dias", "⏳ 3 Dias", "🗓️ Todos"]
            for i, f in enumerate(filtros):
                if c_cols[i].button(labels[i]): 
                    st.session_state.filtro_f = f; st.rerun()
            ...
            if not df_c.empty:
                ...
                for _, r in df_c.iterrows():
                    if st.checkbox(f"{r['nome']} ({r['dias_res']} dias)", value=sel_all, key=f"sel_{r['id']}"):
                        clientes_marcados.append(r.to_dict())
                
                if st.button("🚀 INICIAR FILA COM SELECIONADOS", type="primary"):
            ...
            # --- MODO FILA ATIVO (Um por um) ---
            fila = st.session_state.clientes_selecionados
            idx = st.session_state.indice_fila
            if idx < len(fila):
                r = fila[idx]
                st.info(f"Fila: {idx + 1} de {len(fila)}")
                st.markdown(f'<div class="cliente-card-html"><div class="info-container"><div class="nome-c">{r["nome"]}</div><span>{r["whatsapp"]}</span></div></div>', unsafe_allow_html=True)
                
                cnpj_pix = "\n\n💠PIX CNPJ\n62.326.879/0001-13\n\n⚠️ NÃO ESQUEÇA O COMPROVANTE!"
                msg_map = {"vencidos": "🚨SUA ASSINATURA VENCEU !" + cnpj_pix, "hoje": "⚠️VENCE HOJE ⏰!" + cnpj_pix, "1dia": "⚠️VENCE AMANHÃ ⏰!" + cnpj_pix, "2dias": "⚠️VENCE EM 2 DIAS ⏰!" + cnpj_pix, "3dias": "⚠️VENCE EM 3 DIAS ⏰!" + cnpj_pix, "todos": "Lembrete SUPERTV4K."}
                msg = msg_map.get(st.session_state.filtro_f, msg_map["todos"])
                url = f"https://wa.me/55{r['whatsapp']}?text={urllib.parse.quote(msg)}"
                
                col1, col2, col3 = st.columns(3)
                if col1.link_button("📲 ENVIAR", url, type="primary"): st.session_state.em_disparo = True
                if col2.button("⏭️ PULAR"): st.session_state.indice_fila += 1; st.rerun()
                if col3.button("✖️ PARAR"): st.session_state.modo_fila_ativo = False; st.rerun()

                if st.session_state.em_disparo:
                    st.warning("⚠️ Aguarde 10 segundos...")
                    barra = st.progress(0)
                    for t in range(10):
                        time.sleep(1); barra.progress((t + 1) * 10)
                    st.session_state.indice_fila += 1
                    st.session_state.em_disparo = False
                    st.rerun()
            else:
                st.success("✅ Fila concluída!"); st.session_state.modo_fila_ativo = False
